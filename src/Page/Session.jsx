...
export default function Sessions() {
  ...
  const getDeviceIcon = (type) => {
    ...
    if (t.includes("mobile") || t.includes("phone")) return <PhoneIcon sx={{ color: "#a7f3d0" }} />
  ...
  return (
    ...
    >
      <Box sx={{ maxWidth: 880, mx: "auto" }}>
        <Paper elevation={0} sx={{ p: 4, borderRadius: 4, color: "#1f2937", background: "#ffffff", border: "1px solid rgba(6,95,70,0.1)" }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button variant="outlined" onClick={() => navigate("/dashboard")} sx={{ color: "#059669", borderColor: "#059669" }}>
                ← Back
              </Button>
              <Button
                variant="outlined"
                ...
                sx={{ color: "#059669", borderColor: "#059669", "@keyframes spin": { from: { transform: "rotate(0deg)" }, to: { transform: "rotate(360deg)" } } }}
              >
                Refresh
              </Button>
            </Box>
          ...
          <Box sx={{
            ...
            <Typography sx={{ fontWeight: 700, mb: 0.5, color: "#065f46" }}>Session Limit: 2 devices maximum</Typography>
            <Typography variant="body2" sx={{ color: "#374151" }}>
              When you sign in from a 3rd device, the oldest session will be logged out after 1 minute.
            </Typography>
          </Box>
              ...
              {sessions.map((session) => (
                <Card
                  ...
                  sx={{
                    ...
                    border: session.isCurrent ? "1.5px solid #22c55e66" : "1px solid rgba(6,95,70,0.1)",
                    borderRadius: 3,
                    color: "#1f2937",
                  }}
                >
                  <CardContent>
                    <Box sx={{ display: "flex", gap: 2, alignItems: "stretch" }}>
                      <Box sx={{ width: 56, display: "flex", alignItems: "center", justifyContent: "center", background: "#ecfdf5", borderRadius: 2 }}>
                        {getDeviceIcon(session.deviceInfo?.type)}
                      </Box>
                      <Box sx={{ flex: 1 }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
                          <Typography variant="h6" sx={{ color: "#1f2937", fontWeight: 700 }}>
                            {getBrowserIcon(session.deviceInfo?.browser)} {session.deviceInfo?.browser || "Unknown Browser"}
                          </Typography>
                          <Typography variant="body2" sx={{ color: "#065f46" }}>
                            {session.deviceInfo?.os || "Unknown OS"}
                          </Typography>
                          {session.isCurrent && <Chip label="Current Session" size="small" sx={{ backgroundColor: "#dcfce7", color: "#065f46" }} />}
                        </Box>

                        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 1.5 }}>
                          {session.deviceInfo?.city || session.deviceInfo?.country ? (
                            <Typography variant="body2" sx={{ display: "flex", alignItems: "center", gap: 1, color: "#374151" }}>
                              <LocationIcon sx={{ fontSize: 18, color: "#059669" }} />
                          ...
                          {session.deviceInfo?.ip ? (
                            <Typography variant="body2" sx={{ display: "flex", alignItems: "center", gap: 1, color: "#374151" }}>
                              <PublicIcon sx={{ fontSize: 18, color: "#059669" }} />
                              {session.deviceInfo?.ip}
                            </Typography>
                          ) : null}

                          <Typography variant="body2" sx={{ display: "flex", alignItems: "center", gap: 1, color: "#374151" }}>
                            <AccessTimeIcon sx={{ fontSize: 18, color: "#059669" }} />
                      ...
                      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1, minWidth: 130 }}>
                        {!session.isCurrent ? (
                          <Button
                            variant="outlined"
                            color="error"
                            size="small"
                            startIcon={terminatingSession === session.sessionId ? <CircularProgress size={14} /> : <LogoutIcon />}
                        ...
                        {session.deviceInfo?.secure ? (
                          <Typography variant="caption" sx={{ display: "flex", alignItems: "center", gap: 0.75, color: "#166534" }}>
                            <Box sx={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: "#22c55e" }} /> Secure
                          </Typography>
                        ) : null}
                      </Box>
                    </Box>
