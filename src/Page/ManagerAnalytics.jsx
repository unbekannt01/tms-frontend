...
export default function ManagerAnalytics() {
  ...
  const fetchAnalytics = async (tf = timeframe) => {
    try {
      setLoading(true)
      const res = await API.get(`/manager/analytics?timeframe=${tf}`)
  ...
  const statusData = taskDistribution?.byStatus?.map(item => ({
    name: item.status,
    value: item.count
  })) || []
    ...
    >
      <Container maxWidth="lg">
        ...
        <Grid container spacing={3}>
          {/* Overview cards with better visual hierarchy */}
          ...
          {/* Productivity Trends - Enhanced with better labels and legend */}
          ...
          {/* Status Distribution - Pie Chart for better visualization */}
          ...
          {/* Team Performance - Enhanced with progress bars */}
          <Grid item xs={12}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                        <Box>
                          ...
                          <Chip 
                            ...
                            sx={{ 
                              mt: 1,
                              bgcolor: member.onTimeRate >= 80 ? '#d1fae5' : member.onTimeRate >= 60 ? '#fef3c7' : '#fee2e2',
                              color: member.onTimeRate >= 80 ? '#047857' : member.onTimeRate >= 60 ? '#b45309' : '#b91c1c',
                              fontWeight: 600
                            }}
                          />
                        </Box>
                ...
                {teamPerformance.length === 0 && (
                  <Grid item xs={12}>
                    <Box sx={{ textAlign: 'center', py: 4 }}>
                      <Typography color="text.secondary">No team performance data available for this timeframe.</Typography>
                    </Box>
                  </Grid>
                )}
              </Grid>
